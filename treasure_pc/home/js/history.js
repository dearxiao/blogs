...
$("#JchooseDate").find(".left_gray,.right_gray").click(function() {
	...
	$("#JchooseDate li").on('click',function(e) {
		...
		location.href = "/ucenter/history?recent=" + y + "-" + m;
	});
});
...
$(".lookhis_detail").on("click",function(){
	var orid = $(this).attr("orederid"),bflag=false;
	if(bflag) return;
	bfag = true;
	$.ajax({
	  	type:"get",
	  	...
	  	success:function(e){	
	  		if(e.code == "0"){
	  			$("#dd_a").text(e.data.dealAmount*10000 + "元")
	  			$("#dd_b").text(e.data.stockName + "(" + e.data.stockCode + ")")
	  			$("#dd_c").text(e.data.dealQuantity + "股")
	  			$("#dd_d").text(e.data.dealPrice)
	  			$("#dd_e").text(e.data.createTime)
	  			$("#dd_f").text(e.data.sellPrice)
	  			$("#dd_g").text(e.data.sellTime)
	  			$("#dd_h").text(e.data.profit)
	  			$("#dd_i").text(e.data.profitSelf)
	  			$("#dd_j").text(e.data.publicFee)
	  			$("#dd_k").text(e.data.delayDays != null ? e.data.delayDays : "0")
	  			$("#dd_l").text(e.data.delayFeeSum);
	  			$(".dealdetail").show();
	  		}else{
