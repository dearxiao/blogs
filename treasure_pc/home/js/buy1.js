...
function selectAShare(dom) {
	dom.on('click', function (e) {
		...
		$("#stockName").html(ashareName + '(' + ashareCode + ')');
		//隐藏搜索框，值清空
		cue.hide();
		$("#searchTxt1").val("");
...
function getTimeLine() {
	$.post("/quotation/getTimeLine", {
		...
		$.each(arr, function () {
			...
			//avg_price:6.456,amount:6.45,time:"09:30",totalAmount:7046840.3080199985,totalVolume:1094569.7899999998,volume:32666.5
			if (price >= 0) {
				//现在日期2017-07-18
...
function getStockInfo() {
	...
	$.post("/quotation/getStockInfo", {
		...
		if (nowPrice > map.nowPrice) { //比较最新价与原来的价格
			$(".stock-name .up-arrow-box").hide();
			$(".stock-name .down-arrow-box").css("display", "inline-block");
		} else if (nowPrice < map.nowPrice) {
			$(".stock-name .up-arrow-box").css("display", "inline-block");
			$(".stock-name .down-arrow-box").hide();
		}
		...
		$("#stock-info li > span.r ").each(function (i, o) {
			if (bs[i] == 'swing') {
				...
				$(o).html(Number(map[bs[i]] / 10000).toFixed(4) + "万");
			} else {
		...
		if (Number(map.openPrice).toFixed(2) == '0' || Number(map.openPrice).toFixed(2) == '0.00') {
			$('#btn_buy').html(map.remark).attr('disabled', true).css({
				'background': '#767679'
			});
			stopShare = false;
			updateMoneyRate();
...
$("#buy_price_ul > li").click(function (e) {
	...
	//  $("#check-surplus_ul>li").html(amount * 5000);
	//  $("#stop-loss_ul>li:eq(0)").html(amount * -1000);
	//  $("#stop-loss_ul>li:eq(1)").html(amount * -1333);
	//  $("#stop-loss_ul>li:eq(2)").html(amount * -1700);
	//  $("#publicFee").html(amount * publicFee );
	//  $("#delay_fee").html(amount * delayFee);
	//
	//	
...
$("#popup-confirm-btn").click(function (e) {
	...
	// $.post("/user/stockBuy", params, function (data) {
	// 	if (data.code == '0') {
	// 		tool.popup_err_msg("交易成功");
	// 		location.href = "/ucenter/sell";
	// 	} else {
