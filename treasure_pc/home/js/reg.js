...
var reg = {
    ...
    eventsBind: function () {
        ...
        $("#mobileno").on("keyup afterpaste change", function () {
            ...
            w.addClass("capcha-total-down");
            $("#auth_reg_smsA").removeClass("active");
            if ($("#mobileno").val().length == 11) {
                w.removeClass("capcha-total-down");
            ...
            $("#txt_valid_code").unbind("keyup").bind("keyup", function () {
                ...
                if (_val.length == 4) {
                    $.ajax({
                        ...
                        success: function (data) {
                            $("#txt_valid_code").val("");
                            $('#forgot_passImg').attr('src', '/captcha/imgCode?' + Math.random());
                            if (data.code == '0') {
                                //隐藏img弹窗
                                ...
                                $("#auth_reg_smsA", w).text("60s后重新发送");
                                var allCount = 60;
                                var getCodeCounter = window.setInterval(function () {
                                    var w = $(".capcha-total-down:visible").eq(0);
                                    var s = allCount--;
                                    if (s > 0) {
                                        $("#auth_reg_smsA", w).html(s + "s后重新发送");
                                    } else {
                                        window.clearInterval(getCodeCounter)
                                        w.removeClass("capcha-total-down");
                                        //启用btn
                                        $(".btn-get-capcha", w).addClass("active").html("获取校验码");
                                    }
                                }, 1000);
                            } else {
